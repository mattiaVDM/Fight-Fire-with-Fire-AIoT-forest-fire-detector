...
with open( path+'/frames2.csv', 'w+', encoding='UTF8') as f:
    writer = csv.writer(f)
    line = ('start','end','type')
    writer.writerow(line)
    for i in range(0,len(rexpected),1):
        datetime = datetime.combine(datetime.date(2022,28,3), rexpected[i].end) - datetime.combine(datetime.date(2022,28,3), rexpected[i].start)
        line = [datetime/2, rexpected[i].type]
        writer.writerow(line)
